 ...
 */
class FByte(val future: Future[Byte]) extends FAny[Byte, FByte]{

    override protected val builder: FBuilder[Byte, FByte] = FByte

    def op[R, FR <: FAny[R, FR], B](r: FAnyCompanion[R, FR])(fb: FAny[B, _])(f: (Byte, B) => R)(implicit ec: ExecutionContext): FR =
        r(future.flatMap(a => fb.future.map(b => f(a, b))))

    def op[R, FR <: FAny[R, FR]](r: FAnyCompanion[R, FR], f: Byte => R)(implicit ec: ExecutionContext): FR =
        r(future.map(f))

    def toFByte  (implicit ec: ExecutionContext): FByte   = this
...
object FByte extends FAnyCompanion[Byte, FByte] {
    override def apply(in: Future[Byte]): FByte = new FByte(in)
}
