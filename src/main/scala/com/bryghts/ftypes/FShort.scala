 ...
 */
class FShort(val future: Future[Short]) extends FAny[Short, FShort]{

    override protected val builder: FBuilder[Short, FShort] = FShort

    def op[R, FR <: FAny[R, FR], B](r: FAnyCompanion[R, FR])(fb: FAny[B, _])(f: (Short, B) => R)(implicit ec: ExecutionContext): FR =
        r(future.flatMap(a => fb.future.map(b => f(a, b))))

    def op[R, FR <: FAny[R, FR]](r: FAnyCompanion[R, FR], f: Short => R)(implicit ec: ExecutionContext): FR =
        r(future.map(f))

    def toFByte(implicit ec: ExecutionContext): FByte = op(FByte, _.toByte)
    def toFShort(implicit ec: ExecutionContext): FShort = this
...
object FShort extends FAnyCompanion[Short, FShort] {
    override def apply(in: Future[Short]): FShort = new FShort(in)
}
