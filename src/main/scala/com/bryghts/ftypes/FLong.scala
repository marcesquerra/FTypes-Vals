 ...
 */
class FLong(val future: Future[Long])(override implicit protected val executionContext: ExecutionContext) extends FAny[Long, FLong]{

    def op[R, FR <: FAny[R, FR], B](r: FAnyCompanion[R, FR])(fb: FAny[B, _])(f: (Long, B) => R): FR =
        r(future.flatMap(a => fb.future.map(b => f(a, b))))

    def op[R, FR <: FAny[R, FR]](r: FAnyCompanion[R, FR], f: Long => R): FR =
        r(future.map(f))
