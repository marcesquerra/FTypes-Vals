...
trait INumeric[A, B, R] extends Numeric[A, B, R] {
    def rem    (x: A, y: B): R
}

object Numeric {

    def apply[A, B, R](fromA: A => R, fromB: B => R, num: scala.Fractional[R]): Numeric[A, B, R] =
        new NumericForFractional[A, B, R](fromA, fromB, num)

    def apply[A, B, R](fromA: A => R, fromB: B => R, num: scala.Integral[R]): INumeric[A, B, R] =
